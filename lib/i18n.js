// Module i18n đơn giản - hỗ trợ EN/VI cho phần public của GymBro.
// Đọc cookie `lang`, expose helper t(key) qua res.locals để dùng trong EJS.
//
// Dùng:
//   <%= t('home.hero.title') %>
//   <%= t('common.greeting', { name: user.username }) %>
//
// Key dùng dot-notation, lookup từ locales/<lang>.json. Fallback về 'vi'
// nếu key thiếu trong file ngôn ngữ đang chọn.
...
const COOKIE_MAX_AGE = 1000 * 60 * 60 * 24 * 365; // 1 năm
...
// Reload locales nếu file thay đổi (dev mode)
...
function translate(lang, key, params) {
    ...
    // không tìm thấy → trả về chính key (giúp dev biết key chưa được dịch)
    return key;
}
...
// GET /lang/:code → set cookie + redirect về trang trước
function setLangRoute(req, res) {
    ...
    res.cookie(COOKIE_NAME, lang, {
        ...
        httpOnly: false, // cho phép JS đọc nếu cần
        sameSite: 'lax',
        path: '/'
    });
